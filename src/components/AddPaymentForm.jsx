...
import React, { useState } from 'react';
import { FaArrowLeft } from 'react-icons/fa';
import { addExpense } from '../../api/expense'; // your API function
...
const AddPaymentForm = ({ onCancel }) => {
  const [formData, setFormData] = useState({
    ...
    amount: '',
    notes: '',
    usedMonth: new Date().toISOString().slice(0, 7), // default to current month YYYY-MM
  });

  const [error, setError] = useState('');
  const [successMessage, setSuccessMessage] = useState('');

  const room_id = "5621d051-2916-4520-bc81-5b40279e9a23"; // can be dynamic
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addExpense({
        ...
        amount: formData.amount,
        notes: formData.notes,
        quantity: formData.number || 1,
        used_month: formData.usedMonth + '-01', // convert YYYY-MM → YYYY-MM-01
      });

      setSuccessMessage('Thêm khoản chi thành công!');
      setError('');
      setFormData({ title: '', amount: '', number: 1, notes: '', usedMonth: new Date().toISOString().slice(0, 7) });
    ...
    <div className="max-w-4xl mx-auto p-8 bg-gradient-to-br from-green-200 via-green-100 to-white shadow-xl rounded-lg">
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-y-8">
          ...
          {/* Month Picker */}
          <div className="flex flex-col">
            <label htmlFor="usedMonth" className="text-xl text-green-700 mb-2">Tháng áp dụng</label>
            <input
              type="month"
              id="usedMonth"
              name="usedMonth"
              value={formData.usedMonth}
              onChange={handleChange}
              required
              className="px-6 py-5 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 bg-white shadow-lg transition duration-300 ease-in-out text-lg"
            />
